...
object Huffman {
  ...
  // Part 1: Basics
  /*
  Calculate total weight of tree
   */
  ...
  /*
  Aggregate all characters of tree
   */
  ...
  /*
  Combine two CodeTrees into one
   */
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      freqs sortBy((x) => x._2) map {case (char, weight) => Leaf(char, weight)}
    }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case Nil => trees
      case x :: Nil => trees
      case (x1:CodeTree) :: (x2:CodeTree) :: xs => {
        (makeCodeTree(x1, x2) :: xs) sortWith (weight(_) < weight(_))
      }
    }
   ...
   */
  @tailrec
    def until(isSingleTree: List[CodeTree] => Boolean, combine: List[CodeTree] => List[CodeTree])(initialList: List[CodeTree]): CodeTree = {
      if(isSingleTree(initialList)) initialList.head
      else until(isSingleTree, combine)(combine(initialList))
    }
